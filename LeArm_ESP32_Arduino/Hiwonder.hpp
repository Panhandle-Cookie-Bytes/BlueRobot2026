...
//Start address storage (存放起始地址)
#define MEM_LOBOT_LOGO_BASE					0L	//"LOBOT" base address for identification, used to determine if it's a new FLASH ( "LOBOT"存放基地址，用于识别是否是新FLASH)
#define MEM_FRAME_INDEX_SUM_BASE			4096L//Number of actions in each action group, stored starting from this address; total of 256 action groups (每个动作组有多少动作，从这个地址开始存放，共计256个动作组)
#define MEM_ACT_FULL_BASE					8192L//Action group files start from this address (动作组文件从这个地址开始存放)

// Size (大小)
#define ACT_SUB_FRAME_SIZE					64L		//One action frame occupies 64 bytes (一个动作帧占64字节空间)
#define ACT_FULL_SIZE						16384L	//16KB, a complete action group set occupies 16KB (16KB,一套完整动作组占16KB字节)
...
typedef enum {
    BUTTON_EVENT_PRESSED = 0x01,           /**< @brief Button pressed (按钮被按下) */
    BUTTON_EVENT_LONGPRESS = 0x02,         /**< @brief Button long pressed (按钮被长按) */
    BUTTON_EVENT_LONGPRESS_REPEAT = 0x04,  /**< @brief Button long press re-trigger (按钮长按重触发) */
    BUTTON_EVENT_RELEASE_FROM_LP = 0x08,   /**< @brief Button released from long press (按钮从长按中松开) */
    BUTTON_EVENT_RELEASE_FROM_SP = 0x10,   /**< @brief Button released from short press (按钮从短按中松开) */
    BUTTON_EVENT_CLICK = 0x20,             /**< @brief Button clicked (按钮被点击) */
    BUTTON_EVENT_DOUBLE_CLICK = 0x40,      /**< @brief Button double clicked (按钮被双击) */
    BUTTON_EVENT_TRIPLE_CLICK = 0x80,      /**< @brief Button triple clicked (按钮被三连击) */
} ButtonEventIDEnum;
...
/* PWM servo master controller (PWM舵机总控制器) */
class PwmServo_t{
    public:
        void init(void);
        ...
        // Check if target position is reached (判断是否到达指定位置)
        bool  is_ready(uint16_t servo_index);
        void deinit(void);
};

//Macro to get lower 8 bits of A (宏函数 获得A的低八位)
#define GET_LOW_BYTE(A) (uint8_t)((A))
//Macro to get higher 8 bits of A (宏函数 获得A的高八位)
#define GET_HIGH_BYTE(A) (uint8_t)((A) >> 8)
//Macro to merge A as high 8 bits and B as low 8 bits into a 16-bit integer (宏函数 以A为高八位 B为低八位 合并为16位整形)
#define BYTE_TO_HW(A, B) ((((uint16_t)(A)) << 8) | (uint8_t)(B))
#define ID_ALL 254  //When ID is 254, broadcast to all servos; can be used to read unknown ID servo info (ID为254时，会向所有舵机进行广播，可用于读取未知ID的舵机信息)
...
/* Bus servo master controller (总线舵机总控制器) */
class BusServo_t{
    public:
