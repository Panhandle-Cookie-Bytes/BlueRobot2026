...
#define APP_PACKET_HEADER                  0x55  /* Communication protocol frame header (通信协议帧头) */
#define APP_TX_DATA_LENGTH                   2  /* Data length from sender excluding frame header (除去帧头的发送端数据长度) */
...
typedef enum
{
	...
	APP_COORDINATE_SET, //Coordinate control (坐标控制)

	APP_ACTION_GROUP_RUN = 6, //Run action group (动作组运行)
	APP_FULL_ACTION_STOP, //Stop running action group (停止正在运行的动作组)
	APP_FULL_ACTION_ERASE, //Erase action group downloaded to control board (将下载到控制板的动作组擦除)
	APP_CHASSIS_CONTROL,// Chassis control (底盘控制)
	APP_SERVO_OFFSET_SET, //Set servo offset (舵机偏差设置)
	APP_SERVO_OFFSET_DOWNLOAD,//Download servo offset (舵机偏差下载)
	APP_SERVOS_RESET, //Reset pose (复位位姿)
	APP_SERVOS_READ, //Read servo angle (读取舵机角度)
	
	APP_ACTION_DOWNLOAD = 25, //Download action group (动作组下载)
	APP_FUNC_NULL
}AppFunctionStatus;
