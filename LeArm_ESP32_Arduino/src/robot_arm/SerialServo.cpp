...
//Parse the received packet information and return (解析接收到的数据包信息，并返回)
...
int BusServo_t::ReadID(void)
{
  ...
  buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
  buf[2] = ID_ALL;  //ID_ALL is 254, meaning broadcast to all servos; can be used to read information from servos with unknown IDs (ID_ALL为254，表示向所有舵机进行广播，可用于读取未知ID的舵机信息)
  buf[3] = 3;
...
//Read temperature alarm threshold (读取温度报警阈值)
