...
/* Used to save the total number of action frames for each action group (用于保存已有的动作组中各自的总动作帧数量) */
...
/* 
 * Servo 6: Left +, Right - (6号舵机 左+ 右-)
 * Servo 5: Forward +, Backward - (5号舵机 前+ 后-)
 * Servo 4: Forward -, Backward + (4号舵机 前- 后+)
 * Servo 3: Forward -, Backward + (3号舵机 前- 后+)
 */
...
/*
 * Servo 2: Right turn -, Left turn + (2号舵机 右转- 左转+)
 */
...
{
	/* Set the action frame count of all action groups to 0, which means erasing all action groups (将所有动作组的动作帧数量设置为0，即代表将所有动作组擦除) */
...
/* Contents of an action frame array (一个的动作帧的数组内容)
 * Number of controlled servos: frame[0] (控制的舵机数量：frame[0])
 * Runtime: frame[1] + frame[2] << 8 (运行时间：frame[1] + frame[2] << 8)
 * Servo ID: frame[3 + i * 3] (舵机id：frame[3 + i * 3])
 * Servo pulse width: frame[4 + i * 3] + frame[5 + i * 3] << 8 (舵机脉宽：frame[4 + i * 3] + frame[5 + i * 3] << 8)
 * 0 - action frame failed to execute, 1 - action frame completed (0-动作帧运行失败 1-动作帧运行完成)     0100 1101 0001
 */
...
uint8_t LeArm_t::action_frame_run(uint8_t action_group_index, uint8_t frame_index)
{
	...
	switch(robot_arm.action_group.frame.status)
	{
		...
		case ACTION_FRAME_RUNNING:
			delay(100);
#if (SERVO_TYPE == TYPE_PWM_SERVO)
      /* Take one servo to check if it has reached the specified position (取一个舵机来判断是否来到达指定位置)*/
	...
	// {
		...
		switch (robot_arm.action_group.status)
		{
			case ACTION_GROUP_START:{
				...
				/* If the number of action frames in this action group is greater than 0, it means the action has already been downloaded (如果该动作组的动作帧数量大于0，则说明已经下载过动作) */
